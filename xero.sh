...
check_gpu_compatibility() {
  ...
  if [[ "$DE" == "hyprland" || "$DE" == "cosmic" ]]; then
    echo -e "\n${YELLOW}⚠️  3D acceleration required for best performance in $DE.${RESET}\n"
    sleep 3
  fi
}
...
install_hypr() {
  ...
  check_vm_environment
  start_point
  install_packages hyprland hypridle hyprland-protocols hyprlock hyprpaper hyprpicker hyprpolkitagent hyprsunset linux-headers pacman-contrib xdg-desktop-portal-hyprland xdg-user-dirs power-profiles-daemon sddm
